...
def plot_geo(gdf: geopandas.GeoDataFrame, fig_location: str = None,
    ...
    happened in a village or not.
            ...
            Parameters:
                    gdf (geopandas.GeoDataFrame): GeoDataFrame with
                    data to plot
                    fig_location (str): File name to save figure
                    show_figure (bool): Whether to show figure
    ...
    gdf[gdf["p5a"] == 1].plot(ax=ax[0],
                              markersize=1).set_title(
                                  "Nehody v JHM kraji: V obci")
    ctx.add_basemap(ax[0], crs=gdf.crs.to_string(),
                    source=ctx.providers.Stamen.TonerLite,
                    zoom=10, alpha=0.9)

    gdf[gdf["p5a"] == 2].plot(ax=ax[1], markersize=1,
                              color="r").set_title(
                    "Nehody v JHM kraji: Mimo obce")
...
def plot_cluster(gdf: geopandas.GeoDataFrame, fig_location: str = None,
    ...
    gdf2 = gdf1.dissolve(by="cluster",
                         aggfunc={"p2a": "count"}).rename(
                            columns=dict(p2a="cnt"))
    ...
    ctx.add_basemap(ax, crs="epsg:3857",
                    source=ctx.providers.Stamen.TonerLite, alpha=0.6)
